...
class GUIGraph:
	def __init__(self,canvas):
		...
		self.canvas.bind("<ButtonPress-2>", self.rightClickObject)
		self.canvas.bind("<B1-Motion>", self.moveObject)
		self.canvas.bind("<B2-Motion>", self.moveObject)
	...
	def rightClickObject(self,event):
		...
		drawnClickedObject= event.widget.find_withtag("current")
		
		if(len(drawnClickedObject)!=0):
			drawnClickedObject = drawnClickedObject[0]
			
			for node in self.canvasNodes:
				if node.isMine(drawnClickedObject):
					oldnode = node
					newNode=NodeDrawing(event.x,event.y,self.canvas,empty=True)
					
					
					self.mouseDelta = node.x-event.x, node.y-event.y
					
					self.currentObject = newNode
					LineDrawing(newNode,oldNode,canvas)
